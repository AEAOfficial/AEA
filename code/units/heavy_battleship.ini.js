exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'heavybattleship') { 
              if(message.author.bot) return; 
              message.channel.send(heavyBattleship); 
	          }
	 });

const heavyBattleship = new Discord.MessageEmbed()
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://camo.githubusercontent.')
   .attachFiles(['./code/units/resources/55.png']) 
   .setImage('attachment://55.png') 
   .setTitle(' Heavy Battleship') 
   .setDescription(' - Long range\n- Can attack Land, Air & Subs\n- Decent HP and DPS') 
   .addField('Price:  6000','\u2800')
   .addField('Health:  4000','\u2800')
   .addField('Range:  290','\u2800')
   .addField('Direct Damage:  0 #no directDamage with targetGround','\u2800')
   .addField('Area Damage:  110','\u2800')
   .addField('Direct Damage:  0 #no directDamage with targetGround','\u2800')
   .addField('Area Damage:  110','\u2800')
   .addField('Direct Damage:  65','\u2800')
   .addField('Direct Damage:  95','\u2800')
   .addField('Move Speed:  0.7','\u2800')
   .addField('Turn Speed:  1.0','\u2800')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://imgur.com/QMVH0Hq.png');
}
exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'heavybattleship') { 
              if(message.author.bot) return; 
              message.channel.send(heavyBattleship); 
	          }
	  });
const heavyBattleship = new Discord.MessageEmbed() 
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://cdn.discordapp.com/icons/606586202942079017/7eafb97b0aa80cecb8e4a9f0a7f87c21.webp?size=128')
   //.attachFiles(['./resources/heavyBattleship.png']) 
   //.setImage('attachment://heavyBattleship.png') 
   .setTitle(' Heavy Battleship') 
   .setDescription(' - Long range\n- Can attack Land, Air & Subs\n- Decent HP and DPS') 
   .addField('Stats','__________________') 
   .addField('Price:  6000','__________________')
   .addField('Health:  4000','__________________')
   .addField('Range:  290','__________________')
   .addField('Direct Damage:  0 #no directDamage with targetGround','__________________')
   .addField('Area Damage:  110','__________________')
   .addField('Direct Damage:  0 #no directDamage with targetGround','__________________')
   .addField('Area Damage:  110','__________________')
   .addField('Direct Damage:  65','__________________')
   .addField('Direct Damage:  95','__________________')
   .addField('Move Speed:  0.7','__________________')
   .addField('Turn Speed:  1.0','__________________')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://cdn.discordapp.com/avatars/287608141191970817/6d82a2d09c9b2323f453abf5bfaaa588.png?size=128'); 
}
