exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'antiairturrett212') { 
              if(message.author.bot) return; 
              message.channel.send(antiAirTurretT212); 
	          }
	 });

const antiAirTurretT212 = new Discord.MessageEmbed()
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://camo.githubusercontent.')
   .attachFiles(['./code/units/resources/216.png']) 
   .setImage('attachment://216.png') 
   .addField('Price:  1800','\u2800')
   .addField('Health:  1400','\u2800')
   .setTitle(' SAM Turret T2') 
   .setDescription(' - Long range AA') 
   .addField('Range:  320','\u2800')
   .addField('Direct Damage:  60','\u2800')
   .addField('Area Damage:  40','\u2800')
   .addField('Move Speed:  0','\u2800')
   .addField('Turn Speed:  0','\u2800')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://imgur.com/QMVH0Hq.png');
}
exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'antiairturrett212') { 
              if(message.author.bot) return; 
              message.channel.send(antiAirTurretT212); 
	          }
	  });
const antiAirTurretT212 = new Discord.MessageEmbed() 
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://cdn.discordapp.com/icons/606586202942079017/7eafb97b0aa80cecb8e4a9f0a7f87c21.webp?size=128')
   //.attachFiles(['./resources/antiAirTurretT212.png']) 
   //.setImage('attachment://antiAirTurretT212.png') 
   .addField('Price:  1800','__________________')
   .addField('Health:  1400','__________________')
   .setTitle(' SAM Turret T2') 
   .setDescription(' - Long range AA') 
   .addField('Stats','__________________') 
   .addField('Range:  320','__________________')
   .addField('Direct Damage:  60','__________________')
   .addField('Area Damage:  40','__________________')
   .addField('Move Speed:  0','__________________')
   .addField('Turn Speed:  0','__________________')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://cdn.discordapp.com/avatars/287608141191970817/6d82a2d09c9b2323f453abf5bfaaa588.png?size=128'); 
}
