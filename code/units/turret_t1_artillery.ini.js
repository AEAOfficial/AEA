exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'aeaturret_t1_artillery') { 
              if(message.author.bot) return; 
              message.channel.send(AEAturret_t1_artillery); 
	          }
	 });

const AEAturret_t1_artillery = new Discord.MessageEmbed()
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://camo.githubusercontent.')
   .attachFiles(['./code/units/resources/226.png']) 
   .setImage('attachment://226.png') 
   .addField('Price:  2700','\u2800')
   .addField('Health:  1100','\u2800')
   .addField('Range:  360','\u2800')
   .addField('Area Damage:  150','\u2800')
   .addField('Area Damage:  40','\u2800')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://imgur.com/QMVH0Hq.png');
}
exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'aeaturret_t1_artillery') { 
              if(message.author.bot) return; 
              message.channel.send(AEAturret_t1_artillery); 
	          }
	  });
const AEAturret_t1_artillery = new Discord.MessageEmbed() 
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://cdn.discordapp.com/icons/606586202942079017/7eafb97b0aa80cecb8e4a9f0a7f87c21.webp?size=128')
   //.attachFiles(['./resources/AEAturret_t1_artillery.png']) 
   //.setImage('attachment://AEAturret_t1_artillery.png') 
   .addField('Price:  2700','__________________')
   .addField('Health:  1100','__________________')
   .addField('Range:  360','__________________')
   .addField('Area Damage:  150','__________________')
   .addField('Area Damage:  40','__________________')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://cdn.discordapp.com/avatars/287608141191970817/6d82a2d09c9b2323f453abf5bfaaa588.png?size=128'); 
}
