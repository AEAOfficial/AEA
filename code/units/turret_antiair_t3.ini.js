exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'antiairturrett312') { 
              if(message.author.bot) return; 
              message.channel.send(antiAirTurretT312); 
	          }
	 });

const antiAirTurretT312 = new Discord.MessageEmbed()
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://camo.githubusercontent.')
   .attachFiles(['./code/units/resources/217.png']) 
   .setImage('attachment://217.png') 
   .addField('Price:  9800','\u2800')
   .addField('Health:  3900','\u2800')
   .addField('Shield:  1500','\u2800')
   .setTitle(' SAM Turret T3') 
   .addField('Range:  400','\u2800')
   .addField('Direct Damage:  200','\u2800')
   .addField('Area Damage:  40','\u2800')
   .addField('Move Speed:  0','\u2800')
   .addField('Turn Speed:  0','\u2800')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://imgur.com/QMVH0Hq.png');
}
exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'antiairturrett312') { 
              if(message.author.bot) return; 
              message.channel.send(antiAirTurretT312); 
	          }
	  });
const antiAirTurretT312 = new Discord.MessageEmbed() 
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://cdn.discordapp.com/icons/606586202942079017/7eafb97b0aa80cecb8e4a9f0a7f87c21.webp?size=128')
   //.attachFiles(['./resources/antiAirTurretT312.png']) 
   //.setImage('attachment://antiAirTurretT312.png') 
   .addField('Price:  9800','__________________')
   .addField('Health:  3900','__________________')
   .addField('Shield:  1500','__________________')
   .setTitle(' SAM Turret T3') 
   .addField('Range:  400','__________________')
   .addField('Direct Damage:  200','__________________')
   .addField('Area Damage:  40','__________________')
   .addField('Move Speed:  0','__________________')
   .addField('Turn Speed:  0','__________________')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://cdn.discordapp.com/avatars/287608141191970817/6d82a2d09c9b2323f453abf5bfaaa588.png?size=128'); 
}
