exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'heavymissileship') { 
              if(message.author.bot) return; 
              message.channel.send(heavyMissileShip); 
	          }
	 });

const heavyMissileShip = new Discord.MessageEmbed()
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://camo.githubusercontent.')
   .attachFiles(['./code/units/resources/56.png']) 
   .setImage('attachment://56.png') 
   .addField('Price:  14000','\u2800')
   .addField('Health:  3000','\u2800')
   .setTitle(' Heavy Missile Ship') 
   .setDescription(' - Can attack land & air\n- Long range') 
   .addField('Shield:  100','\u2800')
   .addField('Range:  390','\u2800')
   .addField('Direct Damage:  30','\u2800')
   .addField('Direct Damage:  65','\u2800')
   .addField('Direct Damage:  70','\u2800')
   .addField('Area Damage:  180','\u2800')
   .addField('Move Speed:  0.5','\u2800')
   .addField('Turn Speed:  0.8','\u2800')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://imgur.com/QMVH0Hq.png');
}
exports.code = function(){ 
    client.on('message', message => {
            if (message.content.toLowerCase() === 'heavymissileship') { 
              if(message.author.bot) return; 
              message.channel.send(heavyMissileShip); 
	          }
	  });
const heavyMissileShip = new Discord.MessageEmbed() 
   .setColor('#1500f7') 
   .setAuthor('Lemons#5444', 'https://imgur.com/6NICs3U.png') 
   .setThumbnail('https://cdn.discordapp.com/icons/606586202942079017/7eafb97b0aa80cecb8e4a9f0a7f87c21.webp?size=128')
   //.attachFiles(['./resources/heavyMissileShip.png']) 
   //.setImage('attachment://heavyMissileShip.png') 
   .addField('Price:  14000','__________________')
   .addField('Health:  3000','__________________')
   .setTitle(' Heavy Missile Ship') 
   .setDescription(' - Can attack land & air\n- Long range') 
   .addField('Stats','__________________') 
   .addField('Shield:  100','__________________')
   .addField('Range:  390','__________________')
   .addField('Direct Damage:  30','__________________')
   .addField('Direct Damage:  65','__________________')
   .addField('Direct Damage:  70','__________________')
   .addField('Area Damage:  180','__________________')
   .addField('Move Speed:  0.5','__________________')
   .addField('Turn Speed:  0.8','__________________')
   .setTimestamp() 
   .setFooter('SkaarjLord', 'https://cdn.discordapp.com/avatars/287608141191970817/6d82a2d09c9b2323f453abf5bfaaa588.png?size=128'); 
}
